...
const { createEmployee, getEmployee, removeEmployee } = require('../../model/employee.model');
const validateToken = require('../middleware/validate')
const { validateEmployee } = require('../../validation/validation');
router.use(bodyParser.json());
router.post('/register', async (req, res, next) => {
    try {
        const employee = req.body;
        const { error } = validateEmployee.validate(employee)
        if (error) {
            res.send("Validation Error!!!: "+error)
        }
        else
        {
